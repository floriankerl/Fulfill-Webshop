    ...
    if request.method == "POST":
        ...
        # if user wants to search based on average review
            ...
            if general_search == "":
                ...
                article = Article.objects.filter(name__icontains=general_search)
...
def article_edit(request, pk: str):
    ...
    if category == "ECS":
        ...
        electronic_form = ElectronicDeviceArticleForm(request.POST or None, instance=article)
...
def article_pdf(request, pk: str):
    ...
    lines = [
        ...
        "Age Restriction: " + str(article.get_ageRestriction_display()),
        "Delivered in: " + article.deliveryIn,
        "Price: " + article.price,
    ]
    # lines.insert("index", "object")

    if article.category == "BKS":
        book = Book.objects.get(related_article_id=int(pk))
        lines[0] = "PDF Download for Book"
        lines.insert(2, "Author: " + book.author)
        lines[4] = "Publisher: " + article.manufacturer
        lines.insert(5, "Pages: " + str(book.pages))
        lines.insert(6, "ISBN: " + str(book.isbn))
    elif article.category == "ECS":
        device = ElectronicDevice.objects.get(related_article_id=int(pk))
        lines[0] = "PDF Download for Electronics"
        lines.insert(4, "Battery: " + device.battery_life)
        lines.insert(5, "Color: " + device.color)
        lines.insert(6, "Material: " + device.material)
    elif article.category == "NUT":
        food = Superfood.objects.get(related_article_id=int(pk))
        lines[0] = "PDF Download for Nutrition"
        lines.insert(4, "Weight: " + food.weight)
        lines.insert(5, "Ingredients: " + food.ingredients)
        lines[6] = "Vegan: " + str(food.vegan)
    elif article.category == "SUP":
        supplement = Supplement.objects.get(related_article_id=int(pk))
        lines[0] = "PDF Download for Supplement"
        lines.insert(4, "Weight: " + supplement.weight)
        lines.insert(5, "Ingredients: " + supplement.ingredients)

    # Draw things on the PDF. Here's where the PDF generation happens.
    # See the ReportLab documentation for the full list of functionality.
    i = 40
    for line in lines:
        lines_formatted = wrap(line, 100)
        for line_formatted in lines_formatted:
            p.drawString(20, i, line_formatted.rstrip())
            i += 20
